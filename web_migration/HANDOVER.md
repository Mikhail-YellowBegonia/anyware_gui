...
### 决策内容
- 旧仓库中 `core/anyware/nonstandard_llm` 的 LLM 模块将被视作“独立智能体”进行单独开发与演进。
- 该模块不会直接被 UI 库包含；未来如需使用，将通过稳定接口/SDK 或服务化方式对接。
- 该模块可迁移至 `kos/llm` 并创建独立仓库，作为后续共享与复用的基础。
 - 该 LLM 模块仍在迁移范围内，但已拆分为新项目；后续做主项目迁移时无需再处理它。

### 决策原因
- 该模块具备高可拓展性与潜在复杂度，早期与 UI 绑定会导致演进受限。
- 当前模块体量轻、耦合弱，适合在此阶段完成边界划分与仓库拆分。
- 有利于长期版本管理、权限控制、依赖隔离与安全策略独立化。
